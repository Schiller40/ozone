<template lang="html">
  <transition :name="transition.name" :mode="transition.mode">
    <router-view v-model="wrapperId" @setTransition="transition = $event" @showCurrentSlideshow="showPrioritizedSlideshow()" :key="$route.params.slideshowId"/>
  </transition>
</template>
...
export default{
  ...
  mounted(){
    ipcRenderer.on('addToVisualStack', (_event, id, lastModified, play = true) => {
  ...
  methods: {
    ...
    addToVisualStack(id, lastModified, play){
      if (typeof lastModified !== 'number' || typeof id !== 'string' || !id || id.length != 64){
    ...
    removeFromVisualStack (id, play){
      ...
      if (play){
        ...
        if (show.changed){
          ...
          console.log(`removed ${id} from the visual stack but wasn't prioritized`);
        }
      }
      if (len === this.visualStack.length + 1)
        return true;
      return false
    }
  }
