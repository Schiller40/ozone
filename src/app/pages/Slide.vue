<template>
  <div class="slide" :id="`slide-${slideno + 1}`" :name="slide.name || false">
    <ShowImage v-if="containsImage" :url="slide.url" :no="slideno" :id="slideshowid" />
    <ShowIframe v-else-if="containsIframe" :url="slide.url" :no="slideno" :id="slideshowid" />
    <ShowVideo
      ...
      :id="slideshowid"
    />
    <ShowText v-if="containsText" :text="slide.text" :no="slideno" :id="slideshowid" />
  </div>
</template>
...
import ShowVideo from '../subpages/ShowVideo.vue'
...
export default class Slide extends Vue {
  ...
  mounted() {
    ...
    if (this.slide.mime === 'text/plain' || this.slide.text) this.containsText = true
  }
